...
main(int argc, char** argv)
{
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZRGB>(argv[1], *tmp) != 0)
	{
		return -1;
	}
	...
	else
	{
		...
		for (int j=0; j < planenormals->points.size();j++){
			...
			if(isnan(planenormals->points[j].normal_x)||isnan(planenormals->points[j].normal_y)||isnan(planenormals->points[j].normal_z))
		...
		for (int j = 0; j < accy.size() ; ++j) {
			...
			norm[1]=accy[j];
			norm[2]=accz[j];
			if(isnan(acos(dot(norm,pmeanNorm)/(sqrt(pow(pmeanNorm[0],2) + pow(pmeanNorm[1],2) + pow(pmeanNorm[2],2))*sqrt(pow(norm[0],2) + pow(norm[1],2) + pow(norm[2],2))))* 180.0 / M_PI))
				norm.clear();
			else{
		...
		for (int i = 0; i <planePoints->points.size(); ++i) {
			...
			if (!isnan(x) && !isnan(y)) {
				//cout<<"ciao\n"<<endl;
				prova.at<cv::Vec3b>(x, y)[0] = 255;
				prova.at<cv::Vec3b>(x, y)[1] = 255;
				prova.at<cv::Vec3b>(x, y)[2] = 255;
				prova.at<cv::Vec3b>(x+1, y)[0] = 255;
				prova.at<cv::Vec3b>(x+1, y)[1] = 255;
				prova.at<cv::Vec3b>(x+1, y)[2] = 255;
				prova.at<cv::Vec3b>(x, y+1)[0] = 255;
				prova.at<cv::Vec3b>(x, y+1)[1] = 255;
				prova.at<cv::Vec3b>(x, y+1)[2] = 255;
				prova.at<cv::Vec3b>(x+1, y+1)[0] = 255;
				prova.at<cv::Vec3b>(x+1, y+1)[1] = 255;
				prova.at<cv::Vec3b>(x+1, y+1)[2] = 255;
				prova.at<cv::Vec3b>(x-1, y)[0] = 255;
				prova.at<cv::Vec3b>(x-1, y)[1] = 255;
				prova.at<cv::Vec3b>(x-1, y)[2] = 255;
				prova.at<cv::Vec3b>(x, y-1)[0] = 255;
		...
		for (std::vector<pcl::PointIndices>::const_iterator i = clusters.begin(); i != clusters.end(); ++i)
		{
			...
			std::string fileName4 = "triangles" + boost::to_string(currentClusterNum) + ".txt";
			std::string fileName5 = "trianglesproj" + boost::to_string(currentClusterNum) + ".txt";
			...
			for (int j=0; j < ncluster->points.size();j++){
				...
				if(isnan(ncluster->points[j].normal_x)||isnan(ncluster->points[j].normal_y)||isnan(ncluster->points[j].normal_z))
			...
			for (int i = 0; i <cluster->points.size(); ++i) {
				...
				if (!isnan(x) && !isnan(y)) {
					//cout<<"ciao\n"<<endl;
					prova.at<cv::Vec3b>(x, y)[0] = 255*msc;
					prova.at<cv::Vec3b>(x, y)[1] = 255*msc;
					prova.at<cv::Vec3b>(x, y)[2] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y)[0] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y)[1] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y)[2] = 255*msc;
					prova.at<cv::Vec3b>(x, y+1)[0] = 255*msc;
					prova.at<cv::Vec3b>(x, y+1)[1] = 255*msc;
					prova.at<cv::Vec3b>(x, y+1)[2] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y+1)[0] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y+1)[1] = 255*msc;
					prova.at<cv::Vec3b>(x+1, y+1)[2] = 255*msc;
					prova.at<cv::Vec3b>(x-1, y)[0] = 255*msc;
					prova.at<cv::Vec3b>(x-1, y)[1] = 255*msc;
					prova.at<cv::Vec3b>(x-1, y)[2] = 255*msc;
					prova.at<cv::Vec3b>(x, y-1)[0] = 255*msc;
			...
			for (int j = 0; j < accy.size() ; ++j) {
				...
				norm[1]=accy[j];
				norm[2]=accz[j];
				if(isnan(acos(dot(norm,normfit)/(sqrt(pow(normfit[0],2) + pow(normfit[1],2) + pow(normfit[2],2))*sqrt(pow(norm[0],2) + pow(norm[1],2) + pow(norm[2],2))))* 180.0 / M_PI))
					norm.clear();
				else{
